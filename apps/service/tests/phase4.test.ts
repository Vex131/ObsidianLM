...
async function makeDataFixture() {
  ...
  await writeFile(
    ...
    JSON.stringify({ ...defaultSettings, managedLlamaPort: 18087 }),
    "utf8",
  );
  return { root, dataDir };
}
async function createFixtureApp(
  ...
  const app = await createServer({
    gpuMonitorOptions: { commandRunner },
    runtimeManagerOptions: {
...
function detectedProcess(
  pid: number,
  role: DetectedProcess["role"],
  ownership: DetectedProcess["ownership"],
  extra: Partial<DetectedProcess> = {},
): DetectedProcess {
  return {
    pid,
    parentPid: null,
...
test("GPU CSV parser maps N/A fields to null without losing valid fields", () => {
  const gpus = parseGpuCsv(
    "0, GPU-NA, NVIDIA Test GPU, 00000000:03:00.0, 555.85, [N/A], 8192, N/A, 8192, [N/A], N/A, N/A, 250.00",
  );
...
test("compute process parser handles empty output and N/A memory", () => {
  ...
  const processes = parseComputeProcessCsv(
    "1234, C:\\llama.cpp\\llama-server.exe, GPU-111, N/A",
  );
...
test("GPU process classification uses proven router roles, child metadata, and excludes previous candidates", () => {
  ...
  const processes = classifyGpuProcesses(rawProcesses, gpus, [
    ...
    detectedProcess(20, "managed_router_child", "proven", {
      parentPid: 10,
  ...
  assert.deepEqual(
    processes.map((process) => process.kind),
    [
      "managed_router",
      "possible_llama_runtime",
      "managed_router_child",
      "possible_managed_router_child",
    ],
  );
...
test("GPU monitor summarizes router memory, deduplicates rows, and retains PID rows on separate GPUs", async () => {
  const status = await getGpuMonitoringStatus(
    ...
    {
      commandRunner: runnerWithOutputs([
  ...
  assert.deepEqual(
    status.processes.map((process) => [process.gpuIndex, process.pid]),
    [
      [0, 10],
      [0, 20],
      [0, 30],
...
test("GPU monitor warns and avoids managed attribution without process awareness", async () => {
  const status = await getGpuMonitoringStatus(null, {
    commandRunner: runnerWithOutputs([
      representativeGpuCsv,
      "10, llama-server.exe, GPU-111, 100",
    ]),
  });
  assert.equal(status.processes[0].kind, "possible_llama_runtime");
  assert.ok(
    status.warnings.some(
      (warning) => warning.code === "process_awareness_unavailable",
    ),
  );
});
test("GPU API reuses proven process awareness for child attribution", async (t) => {
  ...
  const manager = {
    ...
    setGpuStatusSnapshot: () => {},
  } as unknown as RuntimeManager;
  const app = fastify({ logger: false });
  await registerMonitoringRoutes(app, manager, {
    commandRunner: runnerWithOutputs([
      representativeGpuCsv,
      "10, router.exe, GPU-111, 100\n20, llama-server.exe, GPU-222, 8000",
    ]),
  ...
  const response = await app.inject({
    method: "GET",
    url: "/api/monitoring/gpu",
  });
  assert.equal(response.statusCode, 200);
  const body = response.json() as GpuMonitoringStatus;
  assert.deepEqual(
    body.processes.map((process) => process.kind),
    ["managed_router", "managed_router_child"],
  );
...
test("GPU API reports unavailable ownership when process detection returns warnings instead of throwing", async (t) => {
  const manager = {
    refreshProcessAwareness: async () => ({
      processes: [],
      warnings: ["Process detection is unavailable: fixture"],
      detectionMethod: "fixture",
      available: false,
    }),
    setGpuStatusSnapshot: () => {},
  ...
  t.after(async () => app.close());
  const body = (
    await app.inject({ method: "GET", url: "/api/monitoring/gpu" })
  ).json() as GpuMonitoringStatus;
...
test("GET /api/status returns its initialized unavailable GPU snapshot without monitor execution", async (t) => {
  const { app } = await createFixtureApp(t, async () => {
    throw Object.assign(new Error("nvidia-smi failed"), {
      stderr: "GPU query failed",
    });
  });
...
test("GET /api/status uses initialized snapshots without live process or GPU probes", async (t) => {
  ...
  for (const response of responses) {
    assert.equal(response.statusCode, 200);
    const body = response.json();
    assert.deepEqual(Object.keys(body.detection).sort(), [
      "categories",
      "checkedAt",
      "ports",
      "warnings",
    ]);
    assert.deepEqual(Object.keys(body.gpu).sort(), [
      "available",
      "checkedAt",
      "currentManagedRuntimeGpuMemoryMiB",
      "gpuCount",
      "totalMemoryMiB",
      "unknownGpuProcessCount",
      "usedMemoryMiB",
      "warningsCount",
    ]);
    assert.equal(body.gpu.available, false);
