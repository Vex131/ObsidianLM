...
import { getLlamaBuildCapabilities, parseLlamaBuildDevices, parseLlamaBuildHelp, type LlamaBuildProbeRunner } from "../src/discovery/llama-build-capabilities.js";
import { createServer } from "../src/server.js";

test("llama build capability parsers preserve unknown flags and conservative devices", () => {
  const help = parseLlamaBuildHelp(`Usage: llama-server\n  -c, --ctx-size N   Context size (default: 4096)\n  --future-mode {fast,slow}  New mode; environment variable LLAMA_FUTURE\n  --old-flag         Deprecated legacy switch\n    with wrapped details\n  --custom VALUE     Custom setting\n  --`);
  assert.equal(help.flags.length, 4);
  assert.deepEqual(help.flags[0], { canonicalName: "--ctx-size", aliases: ["-c", "--ctx-size"], valuePlaceholder: "N", description: "Context size (default: 4096)", defaultText: "4096" });
  ...
  assert.ok(help.warnings.some((warning) => warning.code === "help_parse_partial"));
  assert.deepEqual(parseLlamaBuildDevices("Available devices:\nCUDA0: NVIDIA RTX\nVulkan0: AMD Radeon\n  malformed device\n1: AMD GPU"), [{ id: "CUDA0", label: "NVIDIA RTX" }, { id: "Vulkan0", label: "AMD Radeon" }, { id: "1", label: "AMD GPU" }]);
});
...
test("capability inspection uses the injected probe runner and caches by executable fingerprint", async () => {
  ...
  const build: DiscoveredLlamaCppBuild = { id: "fixture-build", name: "Fixture", folder: root, serverPath, tools: [], detectedAt: new Date(0).toISOString() };
  let calls = 0;
  const runner: LlamaBuildProbeRunner = async (_path, args) => {
    calls += 1;
    if (args[0] === "--help") return { ok: true, stdout: "  --unknown-flag VALUE  Future setting", stderr: "" };
    if (args[0] === "--list-devices") return { ok: true, stdout: "CUDA0: Test GPU", stderr: "" };
...
test("unknown capability build route returns before any executable probe", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "obsidianlm-capability-route-"));
  ...
  const setup = await app.inject({ method: "POST", url: "/api/auth/setup", payload: { token: "capability-test-token" } });
  assert.equal(setup.statusCode, 201);
  const response = await app.inject({ method: "GET", url: "/api/discovery/llama-builds/not-a-build/capabilities", headers: { authorization: "Bearer capability-test-token" } });
