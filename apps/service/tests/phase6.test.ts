...
async function waitForJob(
  ...
  const job = manager.getJob(id);
  assert.fail(
    `Timed out waiting for job ${id}. Last status: ${job?.status ?? "missing"}`,
  );
}
async function waitForApiJob(
...
async function waitForPersistedJob(dataDir: string, id: string): Promise<void> {
  const deadline = Date.now() + 5000;
  while (Date.now() < deadline) {
    const jobs = JSON.parse(await readFile(path.join(dataDir, "jobs.json"), "utf8")) as JobRecord[];
    if (jobs.some((job) => job.id === id && ["completed", "failed", "cancelled"].includes(job.status))) return;
    await new Promise((resolve) => setTimeout(resolve, 25));
  }
  assert.fail(`Timed out waiting for job ${id} to persist.`);
}
test("jobs.json is created by default storage initialization", async (t) => {
  const { fixture } = await createFixtureApp(t);
  const jobs = JSON.parse(
    await readFile(path.join(fixture.dataDir, "jobs.json"), "utf8"),
  );
...
test("POST /api/jobs/test creates a safe test job", async (t) => {
  ...
  await waitForPersistedJob(fixture.dataDir, body.job.id as string);
});
...
test("failed job command records failed status", async (t) => {
  ...
  const result = await manager.startJob({
    type: "generic",
    executable: path.join(fixture.root, "missing-command"),
  });
...
test("cancelling a running job only targets the current managed child", async (t) => {
  ...
  const job = await waitForJob(
    manager,
    result.job.id,
    (item) => item.status === "cancelled" && item.finishedAt !== null,
  );
...
test("sanitized job responses redact embedded local paths in args and errors", () => {
  const job: JobRecord = {
    ...
    args: [
      "--model=C:\\Users\\name\\model.gguf",
      "--out=/home/name/result.txt",
      "--cache=/data/models/foo.gguf",
      "--workspace=/workspace/results.json",
      "--share=\\\\server\\share\\model.gguf",
    ],
    cwd: "C:\\Users\\name",
    exitCode: 1,
    signal: null,
    ...
    errorMessage:
      "spawn /srv/llama/llama-bench ENOENT after C:\\Users\\name\\missing.exe",
  };
...
test("startup restores queued and running jobs as interrupted failures", async (t) => {
  ...
  await writeFile(
    path.join(fixture.dataDir, "jobs.json"),
    JSON.stringify(jobs),
    "utf8",
  );
  const manager = new JobManager();
  ...
  assert.equal(
    restored.every((job) => job.status === "failed"),
    true,
  );
  assert.match(
    restored.map((job) => job.errorMessage).join(" "),
    /interrupted by service startup/i,
  );
});
test("job logs endpoint redacts unsafe local command details", async (t) => {
  ...
  assert.equal(body.job.executable, path.basename(process.execPath));
  assert.equal(
    new RegExp(fixture.root.replace(/[\\^$.*+?()[\]{}|]/gu, "\\$&")).test(
      JSON.stringify(body),
    ),
    false,
  );
  assert.equal(
    new RegExp(process.execPath.replace(/[\\^$.*+?()[\]{}|]/gu, "\\$&")).test(
      JSON.stringify(body),
    ),
    false,
  );
...
test("job logs endpoint redacts local paths emitted by job output", async (t) => {
  ...
  const result = await manager.startJob({
    ...
    args: [
      "-e",
      `console.log(${JSON.stringify(`${emittedWindowsPath} ${emittedPosixPath} ${emittedDataPath} ${emittedUncPath}`)});`,
    ],
  });
  assert.ok(result.job);
  await waitForJob(
  ...
  const response = await app.inject({
    method: "GET",
    url: `/api/jobs/${result.job.id}/logs`,
  });
  assert.equal(response.statusCode, 200);
