...
import {
  buildLlamaPerplexityCommand,
  validateLlamaPerplexityRequestShape,
} from "../src/tools/llama-perplexity/command-builder.js";
...
async function waitForJob(
  ...
  assert.fail(
    `Timed out waiting for job ${id}; last status: ${manager.getJob(id)?.status ?? "missing"}`,
  );
}
...
test("tool input discovery scans configured folders, filters extensions, skips symlinks, and warns on missing folders", async (t) => {
  ...
  try {
    await symlink(
      path.join(fixture.inputDir, "a.txt"),
      path.join(fixture.inputDir, "linked.txt"),
    );
  } catch {
  ...
  await saveSettings({
    ...defaultSettings,
    toolInputFolders: [fixture.inputDir, path.join(fixture.root, "missing")],
  });

  const discovered = await discoverToolInputs();
  assert.deepEqual(discovered.files.map((file) => file.extension).sort(), [
    ".jsonl",
    ".md",
    ".raw",
    ".txt",
  ...
  assert.equal(
    discovered.files.some((file) => file.fileName === "linked.txt"),
    false,
  );
  assert.equal(
    discovered.files.some((file) => file.fileName === "ignored.csv"),
    false,
  );
  assert.ok(
    discovered.warnings.some((warning) => warning.code === "folder_missing"),
  );
});

test("llama-perplexity command builder and validation allow only safe numeric options", () => {
  const command = buildLlamaPerplexityCommand(
    {
      args: {
        threads: 8,
        ctxSize: 4096,
  ...
  assert.deepEqual(
    validateLlamaPerplexityRequestShape({
      args: { threads: 1, nGpuLayers: 0 },
    }),
    [],
  );
  assert.match(
    validateLlamaPerplexityRequestShape({ args: { threads: 0 } }).join(" "),
    /threads must be an integer/u,
  );
  assert.match(
    validateLlamaPerplexityRequestShape({
      args: { flashAttention: true },
    } as never).join(" "),
    /args\.flashAttention is not supported/u,
  );
  assert.match(
    validateLlamaPerplexityRequestShape({ extraArgs: ["--bad"] } as never).join(
      " ",
    ),
    /extraArgs is not supported/u,
  );
});

test("llama-perplexity parser extracts final and progress estimates and falls back with warnings", () => {
  const parsed = parseLlamaPerplexityOutput(
    "[1]15.2701,[2]5.4007\nFinal estimate: PPL = 5.4007 +/- 0.67339\n",
  );
  assert.equal(parsed.parsed, true);
  assert.equal(parsed.finalPpl, 5.4007);
  assert.equal(parsed.uncertainty, 0.67339);
  assert.deepEqual(parsed.estimates, [
    { index: 1, ppl: 15.2701 },
    { index: 2, ppl: 5.4007 },
  ]);
  ...
  assert.match(
    progressOnly.warnings.join(" "),
    /No final llama-perplexity PPL estimate/u,
  );
});

test("llama-perplexity route validates requests without credentials", async (t) => {
  ...
  const invalidShape = await app.inject({
    method: "POST",
    url: "/api/jobs/llama-perplexity",
    payload: { args: { threads: 0 } },
  });
...
test("llama-perplexity route rejects undiscovered tool, model, and dataset paths", async (t) => {
  ...
  const badTool = await app.inject({
    ...
    url: "/api/jobs/llama-perplexity",
    payload: {
      perplexityPath: path.join(fixture.root, "rogue-perplexity.exe"),
  ...
  const badModel = await app.inject({
    ...
    url: "/api/jobs/llama-perplexity",
    payload: {
      perplexityPath: fixture.perplexityPath,
      modelPath: outsideModel,
      datasetPath: fixture.datasetPath,
    },
  });
  ...
  const badDataset = await app.inject({
    method: "POST",
    url: "/api/jobs/llama-perplexity",
    payload: {
      perplexityPath: fixture.perplexityPath,
