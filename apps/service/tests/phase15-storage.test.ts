...
function asV1(snapshot: any): any {
  ...
  v1.revision = createHash("sha256")
    .update(canonicalJson({ ...v1, revision: undefined, migration }))
    .digest("hex");
  return v1;
}
...
test("Phase 15 migrates compactly, preserves evidence and custom fields, and is deterministic", async (t) => {
  ...
  const alternate =
    process.platform === "win32"
      ? f.model.replaceAll("\\", "/").toUpperCase()
      : f.model;
  const profiles = [
    profile("z", "Duplicate", alternate, f.build, {
      host: "0.0.0.0",
      port: 9001,
      llamaArgs: { ctxSize: 4096 },
    }),
    profile("a", "Duplicate", f.model, f.build, {
      llamaArgs: { ctxSize: 16384, tensorSplit: "3,1" },
      flagOverrides: [{ flag: "--custom", values: ["one", "two"] }],
      extraArgs: ["--future", "preserve"],
    }),
    ...
    profile(
      ...
      "Missing build",
      f.model,
      path.join(f.dir, "missing-server.exe"),
    ),
  ];
  ...
  const mapping = (id: string) =>
    snapshot.migration.mappings.find((m: any) => m.legacyProfileId === id)!;
  ...
  const configuredA = snapshot.configuredModels.find(
    (m: any) => m.displayName === "Duplicate" && m.llamaArgs?.ctxSize === 16384,
  )!;
  const configuredMissing = snapshot.configuredModels.find(
    (m: any) => m.displayName === "Missing",
  )!;
  const configuredMissingBuild = snapshot.configuredModels.find(
    (m: any) => m.displayName === "Missing build",
  )!;
  assert.equal(mapping("z").legacyRuntimeEndpoint.port, 9001);
  assert.equal(configuredA.llamaArgs!.ctxSize, 16384);
  assert.deepEqual(configuredA.flagOverrides, [
    { flag: "--custom", values: ["one", "two"] },
  ]);
  ...
  assert.equal(
    snapshot.builds.find((b: any) => b.id === mapping("a").buildId)
      ?.managedInferenceEligibility,
    "not_validated",
  );
  const duplicateAliases = snapshot.configuredModels
    .filter((m: any) => m.displayName === "Duplicate")
    .map((m: any) => m.routerAlias);
  assert.equal(duplicateAliases[0], "duplicate");
  assert.notEqual(duplicateAliases[0], duplicateAliases[1]);
  assert.equal(
    snapshot.configuredModels.some((m: any) => "projector" in m),
    false,
  );
  ...
  await writeFile(
    path.join(f.dir, "profiles.json"),
    JSON.stringify([...profiles].reverse()),
  );
  ...
  assert.deepEqual(
    rebuilt.configuredModels.map((model: any) => [model.id, model.routerAlias]),
    snapshot.configuredModels.map((model: any) => [
      model.id,
      model.routerAlias,
    ]),
  );
...
test("Phase 15 v2 ignores changed profiles and does not rewrite or create another backup", async (t) => {
  ...
  const backupCount = (await names(f.dir, /^profiles\.json\.phase15-.*\.bak$/))
    .length;
  assert.equal(await migratePhase15Domain(f.dir), "already_migrated");
  assert.deepEqual(
  ...
  await writeFile(
    path.join(f.dir, "profiles.json"),
    JSON.stringify([profile("two", "Two", f.custom, f.build)]),
  );
  assert.equal(await migratePhase15Domain(f.dir), "already_migrated");
  assert.deepEqual(
    await readFile(path.join(f.dir, "phase15-domain.json")),
...
test("Phase 15 upgrades an exact v1 store atomically and only once", async (t) => {
  ...
  await assert.rejects(
    () =>
      migratePhase15Domain(f.dir, {
        rename: async () => {
          throw new Error("upgrade rename failed");
        },
      }),
  ...
  assert.deepEqual(
    upgraded.compatibilityBindings,
    v1.migration.mappings.map((mapping: any) => ({
  ...
  assert.ok(
    (await names(f.dir, /^phase15-domain\.json\.schema-v1-upgrade-.*\.bak$/u))
      .length >= 1,
  );
...
test("Phase 15 backs up malformed or unsupported source and never creates a target", async (t) => {
  for (const [label, content] of [
    ["json", "{"],
    ["shape", JSON.stringify({ profiles: [] })],
    [
      "shape",
      JSON.stringify([
        profile("bad", "Bad", "model.gguf", "server.exe", {
          llamaArgs: { ctxSize: "invalid" } as any,
        }),
      ]),
    ],
  ] as const) {
    ...
    assert.equal(
      await loadLegacyProfiles(f.dir).then((r) => r.kind),
      label === "json" ? "invalid_json" : "unsupported_shape",
    );
    const backups = await names(f.dir, /^profiles\.json\.phase15-.*\.bak$/);
    assert.equal(backups.length, 1);
    assert.equal(
      await readFile(path.join(f.dir, backups[0]!), "utf8"),
      content,
    );
    await assert.rejects(
      () => readFile(path.join(f.dir, "phase15-domain.json")),
      { code: "ENOENT" },
    );
  }
});
...
test("Phase 15 startup sees malformed legacy storage before forgiving recovery", async (t) => {
  ...
  t.after(() => {
    if (previousDataDir === undefined) delete process.env.OBSIDIANLM_DATA_DIR;
    else process.env.OBSIDIANLM_DATA_DIR = previousDataDir;
    if (previousLogDir === undefined) delete process.env.OBSIDIANLM_LOG_DIR;
    else process.env.OBSIDIANLM_LOG_DIR = previousLogDir;
  });
...
test("Phase 15 backs up malformed targets, rejects duplicate aliases/cross references, and preserves the old target", async (t) => {
  for (const bad of [
    ...
    await writeFile(
      path.join(f.dir, "profiles.json"),
      JSON.stringify([profile("new", "New", f.model, f.build)]),
    );
    ...
    assert.equal(
      (await names(f.dir, /^phase15-domain\.json\.corrupt-phase15-.*\.bak$/))
        .length,
      1,
    );
  }
  ...
  duplicateAlias.configuredModels[1].routerAlias =
    duplicateAlias.configuredModels[0].routerAlias;
  assert.throws(
    () => validatePhase15DomainSnapshot(duplicateAlias),
  ...
  assert.throws(
    () => validatePhase15DomainSnapshot(brokenReference),
    /invalid model references/,
  );
  const incompleteModel = structuredClone(valid);
  delete incompleteModel.configuredModels[0].enabled;
  assert.throws(
    () => validatePhase15DomainSnapshot(incompleteModel),
    /malformed records/,
  );
  const unsafeMissingModel = structuredClone(valid);
  unsafeMissingModel.artifacts[0].referenceStatus = "missing";
  unsafeMissingModel.configuredModels.find(
    (model: any) => model.artifactId === unsafeMissingModel.artifacts[0].id,
  ).referenceStatus.artifact = "missing";
  assert.throws(
    () => validatePhase15DomainSnapshot(unsafeMissingModel),
    /inconsistent reference validation state/,
  );
});
...
test("Phase 15 initialization atomic failures clean temp files and retry successfully", async (t) => {
  ...
  for (const scenario of scenarios) {
    ...
    if (scenario === "write")
      deps.writeFile = async (file: string, ...args: any[]) => {
        if (file.endsWith(".tmp")) throw new Error("write failed");
        return writeFile(file, args[0], args[1]);
      };
    if (scenario === "rename")
      deps.rename = async () => {
  ...
  const checkedWrite = async (file: string, ...args: any[]) => {
    if (file.endsWith(".tmp"))
      backupSeen =
        (await names(f.dir, /^profiles\.json\.phase15-.*\.bak$/)).length >= 2;
    return writeFile(file, args[0], args[1]);
  };
  assert.equal(
    await migratePhase15Domain(f.dir, { writeFile: checkedWrite as any }),
    "migrated",
  );
