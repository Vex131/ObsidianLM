...
import {
  buildLlamaBenchCommand,
  validateLlamaBenchRequestShape,
} from "../src/tools/llama-bench/command-builder.js";
...
test("llama-bench request validation rejects invalid values, threadsBatch, and unsupported flags", () => {
  assert.deepEqual(
    validateLlamaBenchRequestShape({
      args: { threads: 1, nGpuLayers: 0, repetitions: 1 },
    }),
    [],
  );
  assert.match(
    validateLlamaBenchRequestShape({ threads: 0 }).join(" "),
    /threads must be an integer greater than or equal to 1/u,
  );
  assert.match(
    validateLlamaBenchRequestShape({ args: { batchSize: 1.5 } }).join(" "),
    /batchSize must be an integer greater than or equal to 1/u,
  );
  assert.match(
    validateLlamaBenchRequestShape({ threadsBatch: 2 } as never).join(" "),
    /threadsBatch is not supported/u,
  );
  assert.match(
    validateLlamaBenchRequestShape({ args: { threadsBatch: 2 } } as never).join(
      " ",
    ),
    /threadsBatch is not supported/u,
  );
  assert.match(
    validateLlamaBenchRequestShape({
      args: { ctxSize: 2048, flashAttention: true },
    } as never).join(" "),
    /args\.flashAttention is not supported/u,
  );
  assert.match(
    validateLlamaBenchRequestShape({
      extraArgs: ["--threads", "99"],
    } as never).join(" "),
...
test("llama-bench result parser reads representative markdown output and reports malformed output", () => {
  const parsed = parseLlamaBenchOutput(
  ...
  assert.match(
    ...
    /No complete llama-bench markdown result table/u,
  );
});
test("llama-bench job route rejects arbitrary benchPath and modelPath outside discovered folders", async (t) => {
  ...
  const manager = new JobManager(
    createMockSpawn("").spawnJob
      ? { spawnJob: createMockSpawn("").spawnJob }
      : {},
  );
  ...
  const badBench = await app.inject({
    method: "POST",
    url: "/api/jobs/llama-bench",
    payload: {
      benchPath: path.join(fixture.root, "rogue-bench.exe"),
  ...
  const badModel = await app.inject({
    method: "POST",
    url: "/api/jobs/llama-bench",
    payload: { benchPath: fixture.benchPath, modelPath: outsideModel },
  });
...
test("job cancellation route does not target unknown processes", async (t) => {
  ...
  const missing = await app.inject({
    method: "POST",
    url: "/api/jobs/not-managed/cancel",
  });
