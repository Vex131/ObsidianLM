...
import {
  detectPort,
  parseWindowsNetstat,
} from "../src/process/port-detector.js";
...
function llamaProcess(
  pid: number,
  commandLine = `C:\\llama.cpp\\llama-server.exe --port 8085`,
): DetectedProcess {
  return {
    pid,
    parentPid: null,
...
test("port owner parsing tolerates unavailable PID info", () => {
  assert.equal(
    parseWindowsNetstat("TCP 127.0.0.1:8085 0.0.0.0:0 LISTENING", 8085),
    null,
  );
});
...
test("classification marks previous running state without process as stale state", () => {
  const summary = classifyRuntimeDetection({
    previousState: runningPreviousState(1234),
    currentManagedPid: null,
    processes: [],
    ports: [],
  });
...
test("classification reports current managed process when PID matches current child", () => {
  const summary = classifyRuntimeDetection({
    previousState: defaultRuntimeState,
    currentManagedPid: 3333,
    processes: [llamaProcess(3333)],
    ports: [],
  });
...
test("classification does not treat the current in-memory child as previous stale state", () => {
  const summary = classifyRuntimeDetection({
    previousState: runningPreviousState(3333),
    currentManagedPid: 3333,
    processes: [llamaProcess(3333)],
    ports: [],
  });
...
test("startup summary treats process detector warnings as unreliable detection", () => {
  const summary = buildDetectionSummary(
    runningPreviousState(7777),
    null,
    {
      processes: [],
      warnings: ["process detection failed"],
      detectionMethod: "test",
    },
    [],
  );
...
test("POST /api/profiles/:id/start rejects legacy compatibility launch without a validated Build", async (t) => {
  ...
  const response = await app.inject({
    method: "POST",
    url: `/api/profiles/${profile.id}/start`,
  });
...
test("Phase 3 read-only API routes return process, port, and detection payloads", async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({ ...defaultSettings, managedLlamaPort: 18086 }),
    "utf8",
  );
  t.after(() => delete process.env.OBSIDIANLM_DATA_DIR);
  const app = await createServer({
