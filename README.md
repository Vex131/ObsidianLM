...
The implemented router integration uses llama.cpp's built-in router/preset capability while preserving ObsidianLM's conservative single-managed-runtime policy. Run 11 real-machine certification is complete, including Home PC B3 duplicated-GGUF identity and B11 external-catalog authority-boundary evidence. Documented B5 Partial, B6/B8 N/A, and B10 Skip outcomes remain truthful non-blocking residuals under the certification policy; see `docs/validation/phase15-run11.md` and personal evidence logs.
