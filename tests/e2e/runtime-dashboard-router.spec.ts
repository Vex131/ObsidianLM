...
test.describe("router runtime dashboard contract", () => {
  test("Runtime shows router authority, drawer state, logs, and launch semantics", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Switch model" }).click(); const drawer = page.getByRole("complementary", { name: "Configured model drawer" }); await expect(drawer).toBeVisible(); await expect(drawer.getByText("Build A").first()).toBeVisible(); await expect(drawer.getByText("Text", { exact: true }).first()).toBeVisible(); await expect(drawer.getByText("Vision", { exact: true }).first()).toBeVisible(); await expect(drawer.getByText("Unknown · explicit projector C:/models/mmproj.gguf")).toBeVisible(); await expect(drawer.getByText("disabled", { exact: true })).toBeVisible(); await expect(drawer.getByText(/validation invalid; artifact missing/)).toBeVisible();
  ...
  test("Dashboard exposes active router metrics, links stopped state to Runtime, and has no child controls", async ({ page }) => {
    ...
    await expect(page.getByText("Build A").first()).toBeVisible(); await expect(page.getByText("Model A").first()).toBeVisible(); await expect(page.getByText("4321").first()).toBeVisible(); await expect(page.getByText(/\d+h \d+m \d+s/).first()).toBeVisible(); await expect(page.getByRole("heading", { name: "Active Runtime Details" })).toBeVisible(); await expect(page.getByText("Text", { exact: true }).first()).toBeVisible(); await expect(page.getByRole("heading", { name: "Resource Snapshot" })).toBeVisible(); await expect(page.getByText("RTX test")).toBeVisible(); await expect(page.getByText("managed_router_child")).not.toBeVisible(); await expect(page.getByText("/api/profiles")).not.toBeVisible();
