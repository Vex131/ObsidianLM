...
const runtime = { state: { status: "running", pid: 1234, activeProfileId: "p1", startedAt: now }, routerState: { status: "running", pid: 1234, activeBuildId: "build-1", configuredModelStates: [{ configuredModelId: "p1", state: "loaded" }] }, warnings: [] };
...
async function fixture(page: Page, options: { gpu?: boolean; storedToken?: boolean } = {}) {
  ...
  await page.route("**/api/**", async (route: Route) => {
    ...
    if (path === "/api/configured-models") return route.fulfill({ json: { revision: 1, configuredModels: [{ id: "p1", displayName: "Fast profile", routerAlias: "fast", artifactId: "artifact-1", buildId: "build-1", enabled: true, artifact: { resource: { locator: model.path }, kind: "model" }, build: { displayName: build.name }, validation: { references: { artifact: "available", build: "available" }, status: "valid" }, warnings: [] }] } });
    if (path === "/api/model-artifacts") return route.fulfill({ json: { revision: 1, artifacts: [{ id: "artifact-1", resource: { owner: { scope: "local" }, locator: model.path }, kind: "model", referenceStatus: "available", configuredModelIds: ["p1"] }] } });
    if (path === "/api/builds") return route.fulfill({ json: { revision: 1, builds: [{ id: "build-1", displayName: build.name, resource: { owner: { scope: "local" }, locator: build.folder }, server: { owner: { scope: "local" }, locator: build.serverPath }, classification: "official", tools: build.tools, managedInferenceEligibility: "not_validated", configuredModelIds: ["p1"], validationInProgress: false }] } });
