...
Builder Runs 1-8 add the versioned domain contract, storage, legacy Profile migration, persistent model/build foundations, bounded functional Build validation, the production derived-preset pipeline, Build/router lifecycle integration, explicit model/Build switching, and conservative process/GPU/log awareness. Phase 15 is not complete.
