...
Builder Runs 1-9 add the versioned domain contract, storage, legacy Profile migration, persistent model/build foundations, bounded functional Build validation, the production derived-preset pipeline, Build/router lifecycle integration, explicit model/Build switching, conservative process/GPU/log awareness, and the configuration-facing operator console. Phase 15 is not complete.
